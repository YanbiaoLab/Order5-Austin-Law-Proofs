...
当前已归档 **120 份有限平凡性证书**。无限侧：**已有模型证书 122 条、已证不存在 4 条、待补证书 4 条**，合计 130 条。E5834/E40037 与 E12294/E33856 不计入待补证书。
<!-- certificate-counts:end -->

<!-- current-proof-status:start -->
**E12294 / E33856 已证明所有模型平凡。** 两份精确 `EquationN → Equation2` 证书分别获得远端 Judge **accepted / true**，公理依赖为空；两题没有非平凡无限模型，不属于 Austin 律。见[数学证明](proofs/Equation12294/PROOF.zh-CN.md)与[验收报告](proofs/validation/eq12294-33856-triviality/README.md)。

**截至 2026-09-15：有限侧剩 10 条待解，无限侧尚有 4 条待补模型证书。** 单元素模型总是存在；有限侧仍需证明全部模型平凡或找到非平凡模型。
...
- **无限侧待补模型证书（4 条）**：[Equation9663](proofs/Equation9663/README.md); [Equation36487](proofs/Equation36487/README.md); [Equation23357](proofs/Equation23357/README.md); [Equation23653](proofs/Equation23653/README.md)。
<!-- current-proof-status:end -->

## 无限模型类型与方程覆盖

截至 **2026-09-17**，按整个仓库已归档模型的载体、运算定义和核心构造方法，可整理为 **9 个构造家族，去重覆盖 122 条方程**。同一类可以包含多套不同规则；原运算和相反运算归入同一家族。这里统计已有证书确认的方程应用，不是模型的同构分类，也不限于当前搜索器后端。

| 无限模型类型 | 已覆盖方程数 | 构造特点 |
|---|---:|---|
| 自由树上的 Step/Code 关系解码 | **42** | 用运算轨迹定义解码关系，证明输出唯一 |
| 自由树上的直接解码／递归逆查询 | **26** | 使用模式匹配、部分逆、像查询或良基递归 |
| 有限模式规则的正规树 | **16** | 给定有限返回规则，限制到正规树载体 |
| 终止汇合重写／商树模型 | **14** | 引入辅助符号，通过商树或唯一规范形定义乘法 |
| 颜色循环正规树 | **6** | 三色、五色、六色模型各覆盖 2 条 |
| 辅助节点与部分逆正规树 | **4** | E10222、E12087 两种构造，各覆盖一对 |
| 行像关系树 | **10** | E21866 构造覆盖 8 条，E23354 构造覆盖 2 条 |
| 辅助列关系与目标关系树 | **4** | E18137、E17286 两种构造，各覆盖一对 |
| 有理数分段线性模型 | **4** | 同一 Le Floch 构造及其相反运算 |
| **合计** | **126 次覆盖，122 条去重方程** | 两组对偶方程各被两个家族覆盖 |

重复覆盖来自 **E5833/E40070**（直接递归模型与 Step/Code 模型）和 **E6878/E39126**（有限规则正规树模型与自由树递归解码模型）。因此各类覆盖数不能直接相加作为方程总数。

同一具体构造的多对复用包括：

- [E21866 四规则行像模型](proofs/Equation21866/Lean/Austin21866/Model.lean)覆盖 **8 条**：E21714/E24200、E21864/E24199、E21865/E24197、E21866/E24201。
- [Le Floch 有理数分段模型](proofs/Equation13102/InfiniteModel.lean)覆盖 **4 条**：E13102/E33273、E20911/E25087。
- 其余可选取 **55 组代表构造，每组对应 2 条**。共 **57 组代表构造应用**覆盖全部 122 条（55×2+8+4）；这不表示已经证明这些模型两两不同构，也不表示它们只能满足所列方程。

其中 **94 条**单列了显式无限性证明，另 **28 条**使用无限归纳树载体但未单列无限性定理。完整编号、历史构造、分类口径及证书入口见[模型分类与覆盖清单](docs/INFINITE-MODEL-FAMILIES.md)和[机器可读统计](docs/infinite-model-families.json)。
...
| [Equation10222](proofs/Equation10222/README.md) | Equation35836 | 20.2 | 已证仅平凡（Lean） | [FiniteTrivial.lean](proofs/Equation10222/FiniteTrivial.lean) | 已校验 | [InfiniteModel.lean](proofs/Equation10222/InfiniteModel.lean) |
| [Equation35836](proofs/Equation35836/README.md) | Equation10222 | 20.2 | 已证仅平凡（Lean） | [FiniteTrivial.lean](proofs/Equation35836/FiniteTrivial.lean) | 已校验 | [InfiniteModel.lean](proofs/Equation35836/InfiniteModel.lean) |
...
| [Equation12087](proofs/Equation12087/README.md) | Equation33884 | 20.2 | 已证仅平凡（Lean） | [FiniteTrivial.lean](proofs/Equation12087/FiniteTrivial.lean) | 已校验 | [InfiniteModel.lean](proofs/Equation12087/InfiniteModel.lean) |
| [Equation33884](proofs/Equation33884/README.md) | Equation12087 | 20.2 | 已证仅平凡（Lean） | [FiniteTrivial.lean](proofs/Equation33884/FiniteTrivial.lean) | 已校验 | [InfiniteModel.lean](proofs/Equation33884/InfiniteModel.lean) |
...
| [Equation18137](proofs/Equation18137/README.md) | Equation27863 | 20.2 | 已证仅平凡（Lean） | [FiniteTrivial.lean](proofs/Equation18137/FiniteTrivial.lean) | 已校验（Lean + Judge） | [InfiniteModel.lean](proofs/Equation18137/InfiniteModel.lean) |
| [Equation27863](proofs/Equation27863/README.md) | Equation18137 | 20.2 | 已证仅平凡（Lean） | [FiniteTrivial.lean](proofs/Equation27863/FiniteTrivial.lean) | 已校验（Lean + Judge） | [InfiniteModel.lean](proofs/Equation27863/InfiniteModel.lean) |
...
| [Equation12294](proofs/Equation12294/README.md) | Equation33856 | 20.3 | 已证仅平凡（Lean） | [FiniteTrivial.lean](proofs/Equation12294/FiniteTrivial.lean) | [已证不存在](proofs/Equation12294/Triviality.lean) | 不可能存在 |
| [Equation33856](proofs/Equation33856/README.md) | Equation12294 | 20.3 | 已证仅平凡（Lean） | [FiniteTrivial.lean](proofs/Equation33856/FiniteTrivial.lean) | [已证不存在](proofs/Equation33856/Triviality.lean) | 不可能存在 |
...
| [Equation17286](proofs/Equation17286/README.md) | Equation28626 | 20.3 | 已证仅平凡（Lean） | [FiniteTrivial.lean](proofs/Equation17286/FiniteTrivial.lean) | 已校验（Lean + Judge） | [InfiniteModel.lean](proofs/Equation17286/InfiniteModel.lean) |
| [Equation28626](proofs/Equation28626/README.md) | Equation17286 | 20.3 | 已证仅平凡（Lean） | [FiniteTrivial.lean](proofs/Equation28626/FiniteTrivial.lean) | 已校验（Lean + Judge） | [InfiniteModel.lean](proofs/Equation28626/InfiniteModel.lean) |
...
| [Equation21714](proofs/Equation21714/README.md) | Equation24200 | 20.3 | 已证仅平凡（Lean） | [FiniteTrivial.lean](proofs/Equation21714/FiniteTrivial.lean) | 已校验 | [InfiniteModel.lean](proofs/Equation21714/InfiniteModel.lean) |
| [Equation24200](proofs/Equation24200/README.md) | Equation21714 | 20.3 | 已证仅平凡（Lean） | [FiniteTrivial.lean](proofs/Equation24200/FiniteTrivial.lean) | 已校验 | [InfiniteModel.lean](proofs/Equation24200/InfiniteModel.lean) |
| [Equation21864](proofs/Equation21864/README.md) | Equation24199 | 20.3 | 已证仅平凡（Lean） | [FiniteTrivial.lean](proofs/Equation21864/FiniteTrivial.lean) | 已校验 | [InfiniteModel.lean](proofs/Equation21864/InfiniteModel.lean) |
| [Equation24199](proofs/Equation24199/README.md) | Equation21864 | 20.3 | 已证仅平凡（Lean） | [FiniteTrivial.lean](proofs/Equation24199/FiniteTrivial.lean) | 已校验 | [InfiniteModel.lean](proofs/Equation24199/InfiniteModel.lean) |
| [Equation21865](proofs/Equation21865/README.md) | Equation24197 | 20.3 | 已证仅平凡（Lean） | [FiniteTrivial.lean](proofs/Equation21865/FiniteTrivial.lean) | 已校验 | [InfiniteModel.lean](proofs/Equation21865/InfiniteModel.lean) |
| [Equation24197](proofs/Equation24197/README.md) | Equation21865 | 20.3 | 已证仅平凡（Lean） | [FiniteTrivial.lean](proofs/Equation24197/FiniteTrivial.lean) | 已校验 | [InfiniteModel.lean](proofs/Equation24197/InfiniteModel.lean) |

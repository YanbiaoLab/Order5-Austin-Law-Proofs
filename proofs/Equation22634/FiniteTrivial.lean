...
@[reducible] def Equation22634 (G : Type _) [Magma G] : Prop := ∀ (x y z : G), x = (y ◇ (y ◇ y)) ◇ ((x ◇ z) ◇ z)

theorem finite_trivial_dual (G : Type*) [Magma G] [Finite G] (h : Equation22634 G) : Equation2 G := by
  let opposite : Magma G := ⟨fun x y => y ◇ x⟩
  have hd : @ Equation22619 G opposite := by
    intro x y z
    exact h x z y

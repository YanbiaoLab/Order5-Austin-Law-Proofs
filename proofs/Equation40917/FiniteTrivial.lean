...
@[reducible] def Equation40917 (G : Type _) [Magma G] : Prop := ∀ (x y z : G), x = ((((y ◇ x) ◇ y) ◇ z) ◇ y) ◇ y

theorem finite_trivial_dual (G : Type*) [Magma G] [Finite G] (h : Equation40917 G) : Equation2 G := by
  let opposite : Magma G := ⟨fun x y => y ◇ x⟩
  have hd : @ Equation5141 G opposite := by
    intro x y z
    exact h x y z

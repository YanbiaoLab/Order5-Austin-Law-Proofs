...
def result : Goal := by
  ...
  · intro «x» «y» «z»
    change «x» = (@Magma.op G originalMagma «y» (@Magma.op G originalMagma (@Magma.op G originalMagma «z» «x») (@Magma.op G originalMagma «y» (@Magma.op G originalMagma «x» «y»))))
    exact hs «x» «y» «z»
  · exact ht
end submission

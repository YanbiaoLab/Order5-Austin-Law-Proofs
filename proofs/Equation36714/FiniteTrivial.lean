...
@[reducible] def Equation36714 (G : Type _) [Magma G] : Prop := ∀ (x y z : G), x = (((y ◇ y) ◇ z) ◇ (y ◇ x)) ◇ z

theorem finite_trivial_dual (G : Type*) [Magma G] [Finite G] (h : Equation36714 G) : Equation2 G := by
  let opposite : Magma G := ⟨fun x y => y ◇ x⟩
  have hd : @ Equation9384 G opposite := by
    intro x y z
    exact h x z y

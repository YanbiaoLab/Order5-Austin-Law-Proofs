...
@[reducible] def Equation11280 (G : Type _) [Magma G] : Prop := ∀ (x y z : G), x = y ◇ ((y ◇ (z ◇ y)) ◇ (x ◇ y))

theorem finite_trivial (G : Type*) [Magma G] [Finite G] (h : Equation11280 G) : Equation2 G := by
  intro a b
  ...
  have ri (t : G) : Function.Injective (fun w : G => w ◇ t) := by
    ...
    exact ⟨x, hx⟩
  exact ri a (li a (ri (a ◇ a) (li a ((h a a a).symm.trans (h a a b)))))

@[reducible] def Equation34778 (G : Type _) [Magma G] : Prop := ∀ (x y z : G), x = ((y ◇ x) ◇ ((y ◇ z) ◇ y)) ◇ y

theorem finite_trivial_dual (G : Type*) [Magma G] [Finite G] (h : Equation34778 G) : Equation2 G := by
  let opposite : Magma G := ⟨fun x y => y ◇ x⟩
  have hd : @ Equation11280 G opposite := by
    intro x y z
    exact h x y z

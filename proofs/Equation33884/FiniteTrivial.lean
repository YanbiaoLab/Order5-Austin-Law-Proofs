...
@[reducible] def Equation33884 (G : Type _) [Magma G] : Prop := ∀ (x y z : G), x = ((y ◇ x) ◇ (y ◇ (x ◇ z))) ◇ z

theorem finite_trivial_dual (G : Type*) [Magma G] [Finite G] (h : Equation33884 G) : Equation2 G := by
  let opposite : Magma G := ⟨fun x y => y ◇ x⟩
  have hd : @ Equation12087 G opposite := by
    intro x y z
    exact h x z y

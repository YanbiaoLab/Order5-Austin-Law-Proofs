...
@[reducible] def Equation41239 (G : Type _) [Magma G] : Prop := ∀ (x y z : G), x = ((((y ◇ z) ◇ y) ◇ x) ◇ y) ◇ y

theorem finite_trivial_dual (G : Type*) [Magma G] [Finite G] (h : Equation41239 G) : Equation2 G := by
  let opposite : Magma G := ⟨fun x y => y ◇ x⟩
  have hd : @ Equation5066 G opposite := by
    intro x y z
    exact h x y z

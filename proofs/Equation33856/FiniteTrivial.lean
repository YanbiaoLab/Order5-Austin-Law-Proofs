...
@[reducible] def Equation33856 (G : Type _) [Magma G] : Prop :=
  ∀ x y z : G, x = ((y ◇ x) ◇ (x ◇ (y ◇ z))) ◇ y

theorem finite_trivial_dual (G : Type*) [Magma G] [Finite G]
    (h : Equation33856 G) : Equation2 G := by
  let opposite : Magma G := ⟨fun x y => y ◇ x⟩
  have hop : @Equation12294 G opposite := by
    intro x y z
    exact h x y z
  exact @Equation12294Finite.finite_trivial G opposite inferInstance hop

#print axioms finite_trivial_dual

example (G : Type*) [Magma G] [Finite G]
    (h : ∀ x y z : G, x = ((y ◇ x) ◇ (x ◇ (y ◇ z))) ◇ y) :
    ∀ x y : G, x = y := finite_trivial_dual G h

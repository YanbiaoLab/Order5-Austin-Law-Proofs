...
@[reducible] def Equation36713 (G : Type _) [Magma G] : Prop := ∀ (x y z : G), x = (((y ◇ y) ◇ z) ◇ (y ◇ x)) ◇ y

theorem finite_trivial_dual (G : Type*) [Magma G] [Finite G] (h : Equation36713 G) : Equation2 G := by
  let opposite : Magma G := ⟨fun x y => y ◇ x⟩
  have hd : @ Equation9345 G opposite := by
    intro x y z
    exact h x y z

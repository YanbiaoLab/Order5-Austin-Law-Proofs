...
@[reducible] def Equation24200 (G : Type _) [Magma G] : Prop := ∀ (x y z : G), x = ((y ◇ x) ◇ x) ◇ ((x ◇ z) ◇ z)

theorem finite_trivial_dual (G : Type*) [Magma G] [Finite G] (h : Equation24200 G) : Equation2 G := by
  let opposite : Magma G := ⟨fun x y => y ◇ x⟩
  have hd : @ Equation21714 G opposite := by
    intro x y z
    exact h x z y

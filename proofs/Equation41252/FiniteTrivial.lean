...
@[reducible] def Equation41252 (G : Type _) [Magma G] : Prop := ∀ (x y z : G), x = ((((y ◇ z) ◇ y) ◇ y) ◇ x) ◇ y

theorem finite_trivial_dual (G : Type*) [Magma G] [Finite G] (h : Equation41252 G) : Equation2 G := by
  let opposite : Magma G := ⟨fun x y => y ◇ x⟩
  have hd : @ Equation4952 G opposite := by
    intro x y z
    exact h x y z

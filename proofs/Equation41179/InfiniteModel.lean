...
namespace submission.Austin5093
/- Experimental E5093 primitives; no source law is asserted here. -/
inductive T where
  | e : T
  | k : T → T
  | p : T → T → T
  ...
  | p key (p repeated body) =>
    if he : repeated = key then
      ...
      let primary : Option (Small (p key (p repeated body))) :=
        ...
        | k right =>
          if (inverseWith key right (decode key)).isSome then
            some ⟨right,by simp only [sz]; omega⟩ else none
        | e => none
      ...
      let hitExtra : Option (Small (p key (p repeated body))) :=
        ...
        | none => none
        | some v =>
          have hv : sz v.val < sz (p key (p repeated body)) := by
            have hs := v.property; simp only [sz]; omega
...
def primary (key body : T) : Option (Small (Container key body)) :=
  match body with
  | p x right => if (inv key right).isSome then some ⟨x,by simp only [Container,sz]; omega⟩ else none
  | k right => if (inv key right).isSome then some ⟨right,by simp only [Container,sz]; omega⟩ else none
  | e => none
...
def hitExtra (key body : T) : Option (Small (Container key body)) :=
  ...
  | some v =>
    ...
    | p x other =>
      ...
      | none => none
      | some w => if w.val = body then some ⟨x,by
          have hs := v.property; rw [hshape] at hs; simp only [Container,sz] at hs ⊢; omega⟩ else none
    | _ => none
...
def Primary (key body out : T) : Prop :=
  (∃ right, body = p out right ∧ Range key right) ∨ (body = k out ∧ Range key out)
def RawExtra (key body out : T) : Prop := ∃ other,
  key = p out other ∧ q out key = p out key ∧ Code (p out key) body
def HitExtra (key body out : T) : Prop := ∃ v other,
  Code key v ∧ v = p out other ∧ Code v body
...
theorem primary_sound {key body : T} {o : Small (Container key body)}
  ...
  | p x right =>
    ...
    · rename_i hr
      have ho := congrArg Subtype.val (Option.some.inj h)
      rw [←ho]; exact Or.inl ⟨right,rfl,(range_iff _ _).mpr hr⟩
    · cases h
...
theorem primary_complete {key body out : T} (h : Primary key body out) :
  ...
  all_goals simp only [primary,if_pos ((range_iff _ _).mp hr)]
    ...
    · rename_i x other hv
      ...
      · rename_i w hw
        ...
        · rename_i ho
          ...
          rw [←hx]
          refine ⟨v.val,other,⟨v.property,hd⟩,hv,?_⟩
...
theorem hitExtra_complete {key body out : T} (h : HitExtra key body out) :
  ...
  obtain ⟨v,other,⟨hv,hd⟩,rfl,⟨hb,hw⟩⟩ := h
  refine ⟨by simp only [Container,sz] at hv ⊢; omega,?_⟩
...
theorem raw_hit_disjoint {key body a out : T} (hr : RawExtra key body a) (hh : HitExtra key body out) : False := by
  exact raw_extra_no_code hr hh.choose_spec.choose_spec.1
...
theorem no_code_full_image {key body out : T} (hr : Range key body) : ¬Code (Container key body) out := by
  ...
  · obtain ⟨v,_,hcv,_,hcb⟩ := he
    have hv := code_small hcv
    have hb := code_small hcb
    have he := range_small_eq hcv hr (by omega)
    rw [he] at hb; omega

theorem inner_hit_completion {x y z v w : T} (hv : q z y = v) (hh : Hit x v w) :
  ...
  · rcases hp with hp | ⟨_,hp⟩
    · have he := T.p.inj (hp.symm.trans hshape)
      have hc' : Code (p x y) w := by rw [←he.1,←hp]; exact hc
      obtain ⟨other,hy⟩ := code_shape hc'
      refine Or.inl ⟨other,hy,?_,hc'⟩
      rw [←he.1]; exact hv.trans hp
    · rw [hp] at hshape; cases hshape
  · exact Or.inr ⟨v,rest,hh.choose_spec.2,hshape,hc⟩
...
theorem equation5093 (x y z : T) : x = q y (q y (q y (q x (q z y)))) := by
  let v := q z y
  let w := q x v
  have hv : q z y = v := rfl
  have hw : q x v = w := rfl
  have hc : Code (Container y w) x ∧ q y (q y w) = Container y w := by
    ...
    · refine ⟨code_of_primary ?_,?_⟩
      · rcases hp with hp | ⟨he,hp⟩
        · exact Or.inl ⟨v,hp,⟨z,hv⟩⟩
        · exact Or.inr ⟨by rw [he]; exact hp,by rw [he]; exact ⟨z,hv⟩⟩
      · rw [plain_inner_first_raw ⟨z,hv⟩ hp]
        exact plain_inner_second_raw ⟨z,hv⟩ hp
...
theorem equation41179 (x y z : Carrier) :
    x = opposite (opposite (opposite (opposite (opposite y z) x) y) y) y :=
  equation5093 x y z
theorem infinite_model : ∃ (A : Type) (op : A → A → A) (f : Nat → A),
    (∀ x y z, x = op y (op y (op y (op x (op z y))))) ∧
    (∀ n j, f n = f j → n = j) :=
  ⟨Carrier, mul, embed, equation5093, embed_injective⟩
end submission.Austin5093

namespace submission
abbrev CM := Austin5093.Carrier
namespace CM
theorem tower_injective (n j : Nat)
    (h : Austin5093.embed n = Austin5093.embed j) : n = j :=
  Austin5093.embed_injective n j h
end CM
instance modelMagma : Magma CM := ⟨Austin5093.opposite⟩
end submission
theorem submission : Goal := by
  refine ⟨submission.CM, submission.modelMagma, ?_, ?_⟩
  · intro x y z
    exact submission.Austin5093.equation41179 x y z
  · intro h
    have bad : 0 = 1 := submission.Austin5093.embed_injective 0 1
      (h (submission.Austin5093.embed 0) (submission.Austin5093.embed 1))

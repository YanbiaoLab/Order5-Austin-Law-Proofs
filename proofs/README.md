...
当前归档（2026-09-15）：**120 份有限平凡性证书、122 份非平凡模型证书、94 份显式无限性证明**。两侧证书齐全为 114 条；另 4 条已有所有模型平凡的证明。有限侧剩 10 条待解，无限侧尚有 4 条待补独立证书。本批 E22446 / E22591 的完整模型、显式 Nat 单射与两份远端 ACCEPTED 证书均已归档，详见 [E22446 核验](validation/eq22446-infinite-model/README.md)和 [E22591 核验](validation/eq22591-formal/README.md)。以下早期批次数字保留为历史记录。

E12294 / E33856 已完成无条件平凡性证明，分别获远端 Judge accepted / true，公理依赖为空；见[本批证明与核验](validation/eq12294-33856-triviality/README.md)。
...
本批本地证据、远端回执、索引与双语 README 审计（Python 标准库）：
...
E17286/E28626 的审计入口仍为 `proofs/validation/eq17286-formal/audit_remote.py`。

本批源码的低内存串行 Lean 重编译入口见上述报告。
旧批次的清单和脚本保留各自日期的快照，不代表当前总量。

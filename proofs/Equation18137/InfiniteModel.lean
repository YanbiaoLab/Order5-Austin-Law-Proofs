...
abbrev CM := submission.Equation18137TreeSchema.Tree
noncomputable instance modelMagma : Magma CM := ⟨submission.Equation18137TreeSchema.op⟩
namespace CM
theorem tower_injective (m n : Nat)
    (h : submission.Equation18137TreeSchema.Tree.atom m = submission.Equation18137TreeSchema.Tree.atom n) :
    m = n := submission.Equation18137TreeSchema.atom_injective m n h
end CM
end submission

theorem submission : Goal := by
  refine ⟨submission.CM, submission.modelMagma, ?_, ?_⟩
  · exact submission.Equation18137TreeSchema.source_law_explicit
  · intro h
    exact Nat.noConfusion (submission.Equation18137TreeSchema.Tree.atom.inj
      (h (submission.Equation18137TreeSchema.Tree.atom 0) (submission.Equation18137TreeSchema.Tree.atom 1)))

example : Goal := submission
example (x y z : submission.CM) :
    x = (y ◇ x) ◇ (z ◇ ((x ◇ z) ◇ z)) :=
  submission.Equation18137TreeSchema.source_law_explicit x y z

#print axioms submission
#print axioms submission.CM.tower_injective
#print axioms submission.Equation18137TreeSchema.infinite_model

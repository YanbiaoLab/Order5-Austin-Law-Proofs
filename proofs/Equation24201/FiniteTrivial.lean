...
@[reducible] def Equation24201 (G : Type _) [Magma G] : Prop := ∀ (x y z w : G), x = ((y ◇ x) ◇ x) ◇ ((x ◇ z) ◇ w)

theorem finite_trivial_dual (G : Type*) [Magma G] [Finite G] (h : Equation24201 G) : Equation2 G := by
  let opposite : Magma G := ⟨fun x y => y ◇ x⟩
  have hd : @ Equation21866 G opposite := by
    intro x y z w
    exact h x w z y

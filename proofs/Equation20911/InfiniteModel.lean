...
theorem submission.CM.model : Goal := by
  letI : Magma ℚ := ⟨(fun a b => submission.CM.f b a)⟩
  refine ⟨ℚ, inferInstance, ?_, ?_⟩
  · intro x y z
    change x = submission.CM.f (submission.CM.f z (submission.CM.f x (submission.CM.f x z))) (submission.CM.f y y)
    exact submission.CM.law25087 x z y
  · intro h

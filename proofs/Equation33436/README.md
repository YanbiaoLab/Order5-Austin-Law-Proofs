...
- 原表：20.2
- 方程：`x = (y ◇ (((z ◇ z) ◇ y) ◇ x)) ◇ y`
- 有限侧数学状态：仅平凡（Blueprint）；本次未新增有限侧 Lean 证书。
- 无限侧：**Lean 4.33.1 编译通过，含 Nat 单射证明。**

使用 E12857 相同载体上的反向运算；本题包装定理已单独编译。
...
随后已通过极光云 **judge-v3-repl** 实际验证，结果为 **accepted**。任务 ID：`30f2f241005a49849b4e416111c52582`。见[完整结果](../validation/aurora/Equation33436/latest.json)和[实际提交的单文件证书](../validation/aurora/Equation33436/certificate.lean)。原来的本地编译记录和历史验收状态保留。

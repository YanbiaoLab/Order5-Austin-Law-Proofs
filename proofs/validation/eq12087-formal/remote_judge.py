"""Submit the checked Equation12087 or Equation33884 certificate once; resume by durable job ID.
...
def prepared_request():
    from audit_judge import local_audit
    local_audit()
    manifest = json.loads((HERE / "judge-local/summary.json").read_text())
    ...
    equation = next(e for e in json.loads((ROOT / "proofs/index.json").read_text())["equations"]
                    if e["equation"] == EQ)
    assert equation["formula"] == json.loads((HERE / "summary.json").read_text())["source_laws"][EQ]
    source = ROOT / "proofs" / EQ / "JudgeSubmission.lean"
    payload = {"problem": {"id": "order5-" + EQ + "-to-Equation2", "eq1_id": int(EQ.removeprefix("Equation")), "eq2_id": 2,
...
def main():
    ...
    parser.add_argument("equation", choices=["Equation12087", "Equation33884"])
    parser.add_argument("--submit", action="store_true")
    parser.add_argument("--wait", action="store_true")
    args = parser.parse_args()
    EQ = args.equation
    OUT = ROOT / "proofs/validation/aurora" / EQ
    source, payload, digest = prepared_request()

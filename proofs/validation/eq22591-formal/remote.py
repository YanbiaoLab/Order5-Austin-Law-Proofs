"""Submit the verified E22591 export once and resume its durable remote Judge job."""
import argparse
import datetime
import hashlib
import json
...
from verify import audit_local as audit_export
from export import ROOT, CERTIFICATE as DEST, sha

BASE = 'http://10.220.69.172:8900'
OUT = ROOT/'proofs/validation/aurora/Equation22591'
...
def prepared_request():
    ...
    entry = next(e for e in json.loads((ROOT/'proofs/index.json').read_text())['equations']
                 if e['equation'] == 'Equation22591')
    assert entry['formula'] == 'x = (y ◇ (y ◇ x)) ◇ ((x ◇ x) ◇ z)'
    payload = dict(problem=dict(id='order5-Equation22591-to-Equation2', eq1_id=22591, eq2_id=2,
...
def main():
    ...
    if state_path.exists():
        ...
        state = dict(base_url=BASE, equation='Equation22591', certificate=str(DEST.relative_to(ROOT)),

"""Submit the checked Equation27863 certificate once; resume by durable job ID.
...
OUT = ROOT / "proofs/validation/aurora/Equation27863"
...
def prepared_request():
    ...
    equation = next(e for e in json.loads((ROOT / "proofs/index.json").read_text())["equations"]
                    if e["equation"] == "Equation27863")
    assert equation["formula"] == manifest["source_law"]
    source = ROOT / "proofs/Equation27863/InfiniteModel.lean"
    payload = {"problem": {"id": "order5-Equation27863-to-Equation2", "eq1_id": 27863, "eq2_id": 2,

...
REMOTE = ROOT / "proofs/validation/aurora/Equation18137"


def audit(check_index=True):
    manifest_path = HERE / "summary.json"
    local = json.loads(manifest_path.read_text())
    assert local["status"] == "passed" and local["modules_checked"] == 6
    ...
    assert problem["eq1_id"] == 18137 and problem["eq2_id"] == 2
    ...
    if check_index:
        ...
        assert index["local_formalizations"]["eq18137"]["manifest_sha256"] == sha(manifest_path)
    return {"status": "passed", "equation": "Equation18137", "local_units_checked": 6,

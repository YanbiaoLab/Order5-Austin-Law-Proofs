"""Submit the checked Equation18137 certificate once; resume by durable job ID.
...
OUT = ROOT / "proofs/validation/aurora/Equation18137"
...
def prepared_request():
    ...
    assert manifest["status"] == "passed" and manifest["classification"] == "nontrivial_infinite_model"
    for name, digest in manifest["source_hashes"].items():
        assert sha(ROOT / name) == digest, name
    equation = next(e for e in json.loads((ROOT / "proofs/index.json").read_text())["equations"]
                    if e["equation"] == "Equation18137")
    assert equation["formula"] == manifest["source_law"]
    source = ROOT / "proofs/Equation18137/InfiniteModel.lean"
    payload = {"problem": {"id": "order5-Equation18137-to-Equation2", "eq1_id": 18137, "eq2_id": 2,
